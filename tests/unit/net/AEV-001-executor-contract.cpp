...
// asio::io_context is used ONLY as a coroutine driver in the Task-specific
// tests. It is NOT testing Asio internals — it is a minimal scheduler for
// running C++20 coroutines synchronously in a unit test.
#include <asio.hpp>
...
// Runs a Task<T> synchronously using a local asio::io_context.
// Only used for the Task-specific test cases.
template<typename T>
T run_task(aevox::Task<T> task) {
    asio::io_context ioc;
    T result{};
    asio::co_spawn(ioc,
        [&task, &result]() -> asio::awaitable<void> {
...
template<>
void run_task<void>(aevox::Task<void> task) {
    asio::io_context ioc;
    asio::co_spawn(ioc,
...
TEST_CASE("AEV-001: Executor — listen() on port 0 succeeds", "[net]") {
    ...
    auto result = ex->listen(0, [](std::uint64_t) -> aevox::Task<void> {
        co_return;
    });
