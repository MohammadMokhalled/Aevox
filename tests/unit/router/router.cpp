// Router path matching, parameter extraction, and dispatch — unit tests.
...
aevox::Request make_test_request(aevox::HttpMethod method, std::string_view path)
{
    ...
    std::string raw;
    raw.reserve(pr.method.size() + 1 + path.size() + 11);
    raw += pr.method;
    raw += ' ';
    raw += path;
    raw += " HTTP/1.1\r\n\r\n";
...
TEST_CASE("Router - static path matching", "[router]")
{
...
TEST_CASE("Router - named parameter extraction", "[router]")
{
    SECTION("string param is extracted")
    {
        aevox::Router r;
        r.get("/users/{name}", [](aevox::Request& /*req*/, const std::string& name) {
    ...
    {
        aevox::Router r;
        r.get("/a/{x}/b/{y}", [](aevox::Request&, const std::string& x, const std::string& y) {
            std::string body;
            body.reserve(x.size() + 1 + y.size());
            body += x;
            body += '+';
            body += y;
...
TEST_CASE("Router - wildcard capture", "[router]")
{
    SECTION("wildcard captures tail")
    {
        aevox::Router r;
        r.get("/files/{path...}",
              [](aevox::Request&, const std::string& p) { return aevox::Response::ok(p); });
...
TEST_CASE("Router - handler invocation", "[router]")
{
    ...
    SECTION("sync arity-1 string handler")
    {
        aevox::Router r;
        r.get("/echo/{name}",
              [](aevox::Request&, const std::string& name) { return aevox::Response::ok(name); });
    ...
    {
        aevox::Router r;
        r.get("/repo/{owner}/{num:int}", [](aevox::Request&, const std::string& owner, int num) {
...
TEST_CASE("Router - method not allowed", "[router]")
{
    SECTION("GET-only route returns 405 for POST with Allow header")
    {
        ...
        REQUIRE(allow->find("GET") !=
                std::string_view::npos); // NOLINT(bugprone-unchecked-optional-access)
    }
}
...
TEST_CASE("Router - concurrent dispatch thread safety", "[router]")
{
...
TEST_CASE("Router - group prefix", "[router]")
{
...
TEST_CASE("Router - match priority", "[router]")
{
    ...
    {
        aevox::Router r;
        r.get("/a/{id}", [](aevox::Request&, const std::string& id) {
            return aevox::Response::ok("param:" + id);
        });
        r.get("/a/{rest...}", [](aevox::Request&, const std::string& rest) {
