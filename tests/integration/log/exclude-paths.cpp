// exclude-paths.cpp: excluded paths produce no log lines
...
std::filesystem::path make_temp_log_path()
{
    ...
    return std::filesystem::temp_directory_path() /
           std::format("aevox-test-exclude-paths-{}-{}.log", counter++,
                       std::chrono::steady_clock::now().time_since_epoch().count());
}

} // namespace

TEST_CASE("exclude paths - health produces no log line but api does", "[integration][log]")
{
...

    {
        const TestServer server{port,
                                [&](aevox::App& app) {
                                    app.use(aevox::middleware::logger({
                                        .exclude_paths = std::unordered_set<std::string>{"/health"},
                                    }));
        ...
        const auto health_resp = http_get(port, "/health");
        REQUIRE(health_resp.find("HTTP/1.1 200") != std::string::npos);

        const auto api_resp = http_get(port, "/api/data");
        REQUIRE(api_resp.find("HTTP/1.1 200") != std::string::npos);
    }
    ...
    REQUIRE(content.find("/api/data") != std::string::npos);
    REQUIRE(content.find("/health") == std::string::npos);
}
