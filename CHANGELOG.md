...
- W3C `traceparent` header auto-extraction: valid headers populate `trace_id` and `span_id` in all `req.log` entries automatically (AEV-012)
...
- Logger middleware now uses `req.log` instead of global logger, ensuring `request_id`, `trace_id`, and `span_id` appear in auto-log entries (AEV-012)
