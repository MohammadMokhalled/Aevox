...
- W3C `traceparent` header auto-extraction: valid headers populate `trace_id` and `span_id` in all `req.logger()` entries automatically (AEV-012)
...
- Structured asynchronous logging subsystem (AEV-011): `aevox::log::Logger`, `LogLevel`, `LogConfig`, console and file sinks with JSON/Pretty formatting, runtime level filtering, per-request correlation via `Request::logger()`, and automatic request/response logging via `aevox::middleware::logger()`
...
### Changed
- `Request::logger()` replaces the former public `Request::log` data member, keeping request-scoped logging encapsulated while preserving the same logging behavior (AEV-028)
...
- `request_id` format changed from monotonic counter (`"req-0"`, `"req-1"`, …) to 16-character lowercase hex random ID generated by per-thread `std::mt19937_64` (AEV-012)
- `correlation_id` renamed to `trace_id` in internal `RequestContext`, `LogEntry`, and JSON/pretty log output; `LogField::CorrelationId` deprecated in favour of `LogField::TraceId` (AEV-012)
- Logger middleware now uses `req.logger()` instead of global logger, ensuring `request_id`, `trace_id`, and `span_id` appear in auto-log entries (AEV-012)
